...
export default function Planning({ latLng, interests, nameURLRating }) {
	...
	function handleClick() {
		history.push({
			pathname: '/search/' + latLng.city.replaceAll(' ', '_').trim().toLowerCase() + '/interests',
			state: {
				interests: interests,
				latLng: latLng,
				...
				<div>
					<Col md={12} className='planning '>
						...
						<Col md={12} lg={6} className='mt-3 mt-lg-4 mt-xl-5'>
							<p className='planning_text'>Start planning {latLng && 'for ' + toTitle(latLng.city)}</p>
							<p className='planning_description ml-md-1'>Create a Trip and organize all of your travel ideas</p>
							{latLng && (
								<Button onClick={handleClick} variant='dark' size='lg' className=' ml-md-1 mb-3 mb-lg-0 button_explore'>
									Explore City
								</Button>
							)}
						</Col>
