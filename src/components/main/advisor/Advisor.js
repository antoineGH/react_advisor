...
export default function Advisor() {
	...
	function filterCategories(latLng, category, categoryID) {
		history.push({
			pathname: city.replaceAll(' ', '_').trim().toLowerCase() + '/' + encodeURIComponent(category.toLowerCase().trim().replaceAll(' ', '_')),
			state: {
				interests: interests,
				latLng: latLng,
				category: category,
	...
	return (
		<>
			<LoadingSpinner />
			{hasError && <Error errorMessage='Impossible to find this city, please try again.' />}
			{isLoaded && (
				<AdvisorContainer
