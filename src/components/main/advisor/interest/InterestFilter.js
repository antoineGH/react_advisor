...
export default function InterestsFilter() {
	...
	if (hasError) {
		return (
			<>
				<Container>
					{_.get(location, 'state.latLng') && <BreadCrumb country={country} state={state} latLng={latLng} />}
					<br />
					{category && <p className='hotel_carousel_title'>{toTitle(category)}</p>}
				</Container>
				<Row className='text-center'>
					<Col>
						<p>
							<FontAwesomeIcon size='1x' icon={['fas', 'info-circle']} /> {'  '}Impossible to get the Category Details. Please try again later
						</p>
	...
	if (!isLoaded) {
		...
		return (
			<>
				...
				<Container fluid>{moreInterests && <InterestList interests={moreInterests} />}</Container>
				<Award />
			</>
		)
	}
